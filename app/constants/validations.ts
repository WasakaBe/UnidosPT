...
const validateFullName = (name: string): string | null => {
  const nameRegex = /^[A-Za-zÁÉÍÓÚáéíóúÑñ\s]{3,50}$/ // Letras y espacios, entre 5 y 50 caracteres
  if (!nameRegex.test(name)) {
    return 'El nombre debe contener solo letras y espacios, y tener entre 3 y 50 caracteres.'
  }
  return null
}
